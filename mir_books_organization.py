...
_booksHave = _booksHave[1:10] + [['Bekar', 'Bekar']]

for data in _booksHave:
    _book = data[0]
    _authors = Rest(data)

    _bookSearch = SQLExecute(_sqlConnection, "SELECT bookID FROM books WHERE bookName='" + _book + "'").fetchall()
    if _bookSearch.__len__() == 0:
        print(_book + ' : not found in the table!')
    else:
        print(_book + ' : found in the table. Moving on!!!')

    for _author in _authors:
        _authorSearch = SQLExecute(_sqlConnection,
                                   "SELECT authorID FROM authors WHERE firstName||lastName='" + _author + "'").fetchall();
        if _authorSearch.__len__() == 0:
            print("\t" + _author + " : not found in the table")
        else:
            print("\t" + _author + " : exists in the table. Moving on!!")
    print("Completed working with the book " + _book + "\n")
